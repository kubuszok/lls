 ...
 * (the Native variant, ISS-977/1347), including the NoSuchFileException tolerance in
 * deleteRecursively (within the documented "missing path is a no-op" contract). Identical body
 * to the scalajvm variant (plan §5).
 */
package lowlevel
package io
