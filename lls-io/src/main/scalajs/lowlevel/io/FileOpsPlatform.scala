 ...
 *
 * Provenance: ported from ssg ssg-commons/src/main/scalajs/ssg/commons/io/FileOpsPlatform.scala
 * (ISS-978/1123), Node.js fs-module implementation.
 *
 * This mirrors the JVM/Native implementation operation-for-operation, delegating to the
 * synchronous fs APIs (readFileSync/writeFileSync/existsSync/statSync) that are the Node
 * analogue of java.nio.file.Files.
 *
 * The fs module is acquired through a lazy `require("fs")` so that loading this module in a
 * browser bundle — where `require` is absent — does not crash at module-init time. `isSupported`
 * reflects exactly that: true when Node fs is actually available (the test/runtime environment),
 * false in a browser.
 *
 * Buffer <-> Array[Byte] conversion is byte-exact for binary content: an Int8Array view is laid
 * over the Buffer's backing ArrayBuffer so each signed byte round-trips without reinterpretation.
 *
 * Semantic differences from the JVM/Native implementation (declared honestly):
 *  - A missing-file read throws on JVM/Native as java.nio.file.NoSuchFileException; on Node,
 *    readFileSync throws an ENOENT error that surfaces to Scala as
 *    scala.scalajs.js.JavaScriptException. Both propagate (no swallow), matching the contract
 *    that reading an absent file fails rather than returning empty.
 *  - isDirectory/isRegularFile use a single statSync (no existsSync pre-check) so there is no
 *    window in which the entry can vanish between two calls (e.g. an external process under
 *    watch mode). When statSync fails because the entry is absent or its attributes cannot be
 *    read, the answer is false — exactly the JVM behavior: java.nio.file.Files.isDirectory /
 *    isRegularFile are documented to "return false if the file does not exist or it cannot be
 *    determined whether the file is a directory [/regular file]". The catch is that JVM-parity
 *    semantic, not a swallowed operation failure: it is the type-of-entry question answering
 *    "no" for an unreadable entry.
 */
package lowlevel
package io
...
private[io] object FileOpsPlatform {
  ...
  def isRegularFile(path: FilePath): Boolean =
    ...
    // whether the file is a regular file". This is the type-of-entry question answering "no", not a swallowed
    // operation.
    try
  ...
  /** Last-modified time in milliseconds since the epoch, read from Node's `statSync(...).mtimeMs` (a Double, floored to a Long). The Node analogue of Files.getLastModifiedTime: statSync throws ENOENT
    * (surfaced as js.JavaScriptException) for a missing path, matching the contract that reading the mtime of an absent file fails.
    */
  def lastModifiedTime(path: FilePath): Long =
    fs.statSync(path.pathString).mtimeMs.asInstanceOf[Double].toLong

  /** True when the entry at `p`, examined without dereferencing, is a directory (a directory symlink reports false, matching the JVM/Native `Files.isDirectory(_, NOFOLLOW_LINKS)`).
    */
  ...
  /** Immediate children, sorted by path string for deterministic output across platforms. Node readdirSync throws ENOENT for a missing path and ENOTDIR for a non-directory path, both propagated (no
    * swallow), matching the missing / non-directory contract of the JVM/Native implementation.
    */
  ...
  /** Recursive pre-order descent mirroring the JVM/Native implementation: each level via [[list]] (sorted), directory before its contents, directory symlinks not descended into (lstat tests the link,
    * not its target).
    */
  ...
  /** Byte-exact file copy; copyFileSync overwrites the destination by default (rebuilds re-copy into a destination that may still hold a previous run's files).
    */
  ...
  /** Removes a file or an entire tree; a missing path is a no-op. Symlinks are deleted as links — a directory symlink is unlinked directly (its target is never descended into), giving the clean-build
    * safety property (deleting a directory never reaches outside it through a link). The tree is walked and removed by hand rather than via fs.rmSync(recursive) so the symlink rule is explicit and
    * identical to the JVM and Native implementations.
    */
