 ...
 *
 * Provenance: ported from ssg ssg-commons/src/test/scalajvm/ssg/commons/io/SymlinkTestSupport.scala
 * (ISS-1347).
 */
package lowlevel
package io
...
/** Platform capability probe for the symlink suite: creates a symbolic link at `link` pointing at `target`, returning true if the platform allowed it. JVM uses java.nio.file.Files.createSymbolicLink.
  * The probe is detection-only — its boolean result lets the suite decide whether to run the does-not-follow-symlinks assertion; it never masks a failure of the operation actually under test.
  */
