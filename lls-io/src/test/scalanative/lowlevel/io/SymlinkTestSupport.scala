 ...
 *
 * Provenance: ported from ssg ssg-commons/src/test/scalanative/ssg/commons/io/SymlinkTestSupport.scala
 * (ISS-1347).
 */
package lowlevel
package io
...
/** Platform capability probe for the symlink suite: creates a symbolic link at `link` pointing at `target`, returning true if the platform allowed it. Scala Native routes through
  * java.nio.file.Files.createSymbolicLink (the same API as JVM); if the Native javalib does not implement it the probe returns false and the suite skips the does-not-follow-symlinks assertion on this
  * platform. The probe is detection-only — its boolean result never masks a failure of the operation actually under test.
  */
object SymlinkTestSupport {
  ...
  /** Probes whether the platform honors LinkOption.NOFOLLOW_LINKS for an existing directory symlink. `link` must already exist and point at a directory target. Returns true when
    ...
    * despite NOFOLLOW_LINKS (ssg ISS-1347: Scala Native Windows).
    */
  ...
  /** Combined capability probe: returns true only when this platform can BOTH create directory symlinks AND honors NOFOLLOW_LINKS during walks/deletes. Uses an isolated probe symlink under
    ...
    *
    * On platforms where this returns false (Native-Windows: symlinks work but NOFOLLOW is not honored, ssg ISS-1347), the caller must NOT create a directory symlink — otherwise the fixture teardown's
    * deleteRecursively would recurse through it and throw.
    */
