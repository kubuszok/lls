 ...
 *
 * Provenance: ported from ssg ssg-commons/src/test/scalajs/ssg/commons/io/SymlinkTestSupport.scala
 * (ISS-1347).
 */
package lowlevel
package io
...
/** Platform capability probe for the symlink suite: creates a symbolic link at `link` pointing at `target`, returning true if the platform allowed it. Scala.js under Node uses fs.symlinkSync. The
  * probe is detection-only — its boolean result lets the suite decide whether to run the does-not-follow-symlinks assertion; it never masks a failure of the operation actually under test.
  */
object SymlinkTestSupport {
  ...
  /** Probes whether the platform honors no-follow-link semantics for an existing directory symlink. `link` must already exist and point at a directory target. Returns true when lstatSync reports the
    ...
    * SymlinkTestSupport (ssg ISS-1347).
    */
  ...
  /** Combined capability probe: returns true only when this platform can BOTH create directory symlinks AND honors no-follow-link semantics during walks/deletes. Uses an isolated probe symlink under
    ...
    *
    * On Node all platforms honor lstat correctly, so this returns true whenever symlink creation succeeds. Exists for API parity with the JVM/Native SymlinkTestSupport (ssg ISS-1347).
    */
