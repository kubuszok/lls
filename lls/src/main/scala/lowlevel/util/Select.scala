 ...
 *
 * Migration notes:
 *   Renames: `Comparator<T>` -> `Ordering[T]`; `GdxRuntimeException` -> `IllegalArgumentException`
 *   Convention: `null` -> `Nullable` for lazy instance; companion object for static methods
...
object Select {
  ...
  def selectIndex[T](items: Array[T], mk: MkArray[T], comp: Ordering[T], kthLowest: Int, size: Int): Int = {
    if (size < 1) {
      ...
      throw IllegalArgumentException(s"Kth rank is larger than size. k: $kthLowest, size: $size")
    }
    val idx: Int =
