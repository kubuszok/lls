...
class QuickSelectTest extends munit.FunSuite {
  ...
  test("select finds kth smallest element") {
    ...
    val idx = qs.select(items, MkArray.mkInt, Ordering.Int, 1, items.length)
  ...
  test("select finds 3rd smallest") {
    ...
    val idx = qs.select(items, MkArray.mkInt, Ordering.Int, 3, items.length)
  ...
  test("select finds largest") {
    val qs    = QuickSelect[Int]()
    val items = Array(7, 2, 9, 4, 5)
    val idx   = qs.select(items, MkArray.mkInt, Ordering.Int, items.length, items.length)
  ...
  test("select single element") {
    val qs    = QuickSelect[Int]()
    val items = Array(42)
    val idx   = qs.select(items, MkArray.mkInt, Ordering.Int, 1, 1)
  ...
  test("select two elements") {
    ...
    val idx   = qs.select(items, MkArray.mkInt, Ordering.Int, 1, 2)
  ...
  test("select equal elements") {
    ...
    val idx   = qs.select(items, MkArray.mkInt, Ordering.Int, 2, items.length)
    assertEquals(items(idx), 5)
  }
  ...
  test("select already sorted array") {
    ...
    val idx   = qs.select(items, MkArray.mkInt, Ordering.Int, 4, items.length)
  ...
  test("select large random array") {
    ...
    val idx = qs.select(items, MkArray.mkInt, Ordering.Int, 100, n)
  ...
  test("select with reverse ordering") {
    ...
    val idx = qs.select(items, MkArray.mkInt, Ordering.Int.reverse, 1, items.length)
    assertEquals(items(idx), 9)
  }
  ...
  test("select with subset of array") {
    ...
    val idx = qs.select(items, MkArray.mkInt, Ordering.Int, 2, 4)
    assertEquals(items(idx), 3)
  }
  ...
  test("select can be reused across multiple calls") {
    ...
    val idx1   = qs.select(items1, MkArray.mkInt, Ordering.Int, 1, items1.length)
    assertEquals(items1(idx1), 10)

    val items2 = Array(50, 40, 60)
    val idx2   = qs.select(items2, MkArray.mkInt, Ordering.Int, 3, items2.length)
