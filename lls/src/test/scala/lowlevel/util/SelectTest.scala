...
class SelectTest extends munit.FunSuite {
  ...
  test("select kthLowest=1 returns minimum") {
    ...
    val result = Select.select(items, MkArray.anyRef[java.lang.Integer], intOrd, 1, items.length)
    assertEquals(result.intValue(), 1)
  }

  test("select kthLowest=size returns maximum") {
    val items  = boxed(5, 3, 8, 1, 9, 2)
    val result = Select.select(items, MkArray.anyRef[java.lang.Integer], intOrd, items.length, items.length)
  ...
  test("select kthLowest=3 returns 3rd smallest") {
    ...
    val result = Select.select(items, MkArray.anyRef[java.lang.Integer], intOrd, 3, items.length)
  ...
  test("select median element") {
    ...
    val result = Select.select(items, MkArray.anyRef[java.lang.Integer], intOrd, 4, items.length)
    assertEquals(result.intValue(), 5)
  }

  test("selectIndex kthLowest=1 returns index of minimum") {
    val items = boxed(5, 3, 8, 1, 9, 2)
    val idx   = Select.selectIndex(items, MkArray.anyRef[java.lang.Integer], intOrd, 1, items.length)
    assertEquals(items(idx).intValue(), 1)
  }

  test("selectIndex kthLowest=size returns index of maximum") {
    val items = boxed(5, 3, 8, 1, 9, 2)
    val idx   = Select.selectIndex(items, MkArray.anyRef[java.lang.Integer], intOrd, items.length, items.length)
    assertEquals(items(idx).intValue(), 9)
  }

  test("select single element array") {
    val items = boxed(42)
    assertEquals(Select.select(items, MkArray.anyRef[java.lang.Integer], intOrd, 1, 1).intValue(), 42)
  }

  test("select two element array min and max") {
    val items = boxed(10, 3)
    assertEquals(Select.select(items, MkArray.anyRef[java.lang.Integer], intOrd, 1, 2).intValue(), 3)
    // Items may be rearranged by previous select, reset
    val items2 = boxed(10, 3)
    assertEquals(Select.select(items2, MkArray.anyRef[java.lang.Integer], intOrd, 2, 2).intValue(), 10)
  }

  test("select with custom reverse ordering") {
    val items = boxed(5, 3, 8, 1, 9, 2)
    // With reverse ordering, kthLowest=1 should return the maximum
    val result = Select.select(items, MkArray.anyRef[java.lang.Integer], intOrd.reverse, 1, items.length)
    assertEquals(result.intValue(), 9)
  }
  ...
  test("select throws on empty array (size < 1)") {
    ...
    intercept[IllegalArgumentException] {
      Select.select(items, MkArray.anyRef[java.lang.Integer], intOrd, 1, 0)
    }
  }
  ...
  test("select throws when kthLowest > size") {
    ...
    intercept[IllegalArgumentException] {
      Select.select(items, MkArray.anyRef[java.lang.Integer], intOrd, 4, 3)
    }
  }
  ...
  test("select with subset size") {
    ...
    val result = Select.select(items, MkArray.anyRef[java.lang.Integer], intOrd, 2, 4)
    assertEquals(result.intValue(), 3)
  }

  test("select all equal elements") {
    val items = boxed(5, 5, 5, 5, 5)
    assertEquals(Select.select(items, MkArray.anyRef[java.lang.Integer], intOrd, 1, 5).intValue(), 5)
    val items2 = boxed(5, 5, 5, 5, 5)
    assertEquals(Select.select(items2, MkArray.anyRef[java.lang.Integer], intOrd, 3, 5).intValue(), 5)
    val items3 = boxed(5, 5, 5, 5, 5)
    assertEquals(Select.select(items3, MkArray.anyRef[java.lang.Integer], intOrd, 5, 5).intValue(), 5)
  }
  ...
  test("select large array with quickselect path") {
    ...
    val result = Select.select(items, MkArray.anyRef[java.lang.Integer], intOrd, 50, n)
  ...
  test("selectIndex returns valid index for middle elements") {
    ...
    val idx   = Select.selectIndex(items, MkArray.anyRef[java.lang.Integer], intOrd, 3, 5)
    assertEquals(items(idx).intValue(), 30)
  }

  test("select with string ordering") {
    val items  = Array("cherry", "apple", "banana", "date")
    val result = Select.select(items, MkArray.anyRef[String], Ordering.String, 1, items.length)
  ...
  test("select with string ordering quickselect path") {
    ...
    val result = Select.select(items, MkArray.anyRef[String], Ordering.String, 3, items.length)
